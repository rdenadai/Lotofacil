...
class JsonPropertySerializer(Serializer):
    def serialize(self, queryset, **options):
        ...
        for obj in queryset:
            ...
            # Add to make possible to serialize via json prefetch_related from a object!
            if hasattr(obj, '_prefetched_objects_cache'):
                for name, field in obj._prefetched_objects_cache.iteritems():
                    self.handle_prefetch_related_field(obj, name)
            self.end_object(obj)
            if self.first:
                self.first = False
    ...
    def get_dump_object(self, obj):
        current = {key: item if isinstance(item, dict) or isinstance(item, list) else escape(item) for key, item in self._current.items()}
    ...
    # Serialize the property field of a model
    def handle_property_field(self, obj, name):
        self._current[name] = getattr(obj, name)

    # Serialize all fields from a prefetch_related lookup
    def handle_prefetch_related_field(self, obj, name):
        try:
            relatedManager = getattr(obj, name)
                ...
                for field in model._meta.local_fields:
                    if field.serialize:
                        if field.rel is None:
                            ...
                            if is_protected_type(value):
                                prefetch_fields[field.name] = value
                            else:
                                value = field.value_to_string(model)
                                prefetch_fields[field.name] = value
                if len(prefetch_fields) > 0:
                    prefetch_fields_all.append(prefetch_fields)
            self._current[name] = prefetch_fields_all
        except:
            raise Exception("Prefetch_Related field is missing related_name on Model.")
